        ...
        <Card withBorder radius="lg" padding="sm">
            <Stack gap="xs">
                ...
                <Text size="xs" c="dimmed">
                    {rec.mimeType} • {human(rec.bytes)} • {rec.durationSeconds > 0 ? fmtDuration(rec.durationSeconds) : '—'}
                </Text>
                <Group justify="apart">
                    <Button size="xs" variant="light" leftSection={<IconDownload size={16} />} onClick={() => onDownload(rec)}>
                        Download
                    </Button>
                    <Button size="xs" color="red" variant="subtle" leftSection={<IconTrash size={16} />} onClick={() => onDelete(rec.id)}>
                        Delete
                    </Button>
                </Group>
            </Stack>
...
async function probeDuration(blob, { timeoutMs = 6000 } = {}) {
    return new Promise((resolve, reject) => {
        ...
        video.addEventListener('seeked', () => {
            ...
            if (Number.isFinite(video.duration) && video.duration > 0) {
                ...
                done(null, new Error('Duration not available'));
            }
        }, { once: true });

        video.addEventListener('error', () => {
            clearTimeout(timer);
            done(null, new Error('Failed to read metadata'));
        }, { once: true });
    });
}
/* ---------- main ---------- */
export default function Library() {
    ...
    const [view, setView] = useState(() => localStorage.getItem('lib:view') ?? 'details'); // 'details' | 'cards'
    const [sortBy, setSortBy] = useState('createdAt'); // 'name' | 'bytes' | 'createdAt' | 'updatedAt' | 'durationSeconds' | 'mimeType'
    ...
    const allChecked = selected.size > 0 && sorted.every((r) => selected.has(r.id));
        ...
        <Stack gap="md">
            ...
            <Group gap="xs">
                <Button
                    ...
                    onClick={() => {
                        const recs = sorted.filter((r) => selected.has(r.id));
                            ...
                            {sorted.map((rec) => {
                                ...
                                return (
                                    <Table.Tr key={rec.id} style={{ userSelect: 'none' }}>
                                        <Table.Td>
                                            <Checkbox checked={selected.has(rec.id)} onChange={() => toggleRow(rec.id)} />
                                        </Table.Td>
                                        ...
                                        <Table.Td>
                                            <Text>{rec.durationSeconds > 0 ? fmtDuration(rec.durationSeconds) : '—'}</Text>
                                        </Table.Td>
                                        ...
                                        <Table.Td>
                                            <Group gap="xs">
                                                ...
                                                <Tooltip label="Delete">
                                                    <ActionIcon color="red" variant="light" onClick={() => remove(rec.id)}>
                                                        <IconTrash size={16} />
                                                    </ActionIcon>
                                                </Tooltip>
                                            </Group>
